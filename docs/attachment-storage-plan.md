...
> 状态：本地文件系统第一阶段已完成；对象存储、副本与灾备仍在规划中
> 适用范围：聊天中的图片、视频和普通文件附件  
> 当前基线：SQLite 只保存附件元数据，完整二进制流式写入配置的本地目录

## 1. 背景

第一阶段已经移除完整文件入内存和 SQLite BLOB 链路。上传按 multipart chunk 写入同文件系统
的 staging 文件，`fsync` 后原子发布；下载使用 seek + take 流式响应并支持 HTTP Range。迁移会
先把旧 `attachments.data` 导出成功，再删除该列。当前仍需继续解决以下多后端与运维问题：

- 无法按部署环境选择 SQLite、本地目录或 OSS，也无法为同一附件维护多个副本。
- 在线副本、历史备份、校验、恢复和存储迁移尚未形成独立机制。
- 本地文件和 SQLite 元数据的跨介质事务目前依靠补偿清理，尚无持久任务状态机和对账工具。
...
当前 TOML 已支持上传大小和本地附件目录。以下是后续多后端、策略和敏感字段引用的目标结构：
