...
> 当前基线：附件元数据和完整二进制均保存在 `chat_rooms.db` 的 `attachments` 表中

## 1. 背景

当前附件上传链路会把整个文件读入内存，再将其作为单个 SQLite BLOB 写入
`attachments.data`。下载时，服务端根据 `Range` 请求使用 SQLite `substr` 读取指定区间。
这种实现部署简单，也能让消息与附件在同一事务中提交，适合当前规模，但随着附件数量、
单文件大小和并发增加，会逐渐放大以下问题：

- 主数据库、WAL、备份和恢复耗时随附件数据一起增长。
- 附件上传和下载会与房间、用户、消息读写争用 SQLite 连接和 I/O。
- 目前上传需要完整 `Vec<u8>`，服务端内存使用与文件大小线性相关。
- 无法按部署环境选择 SQLite、本地目录或 OSS，也无法为同一附件维护多个副本。
...
- 不依赖原始文件名作为物理对象键，也不默认进行跨用户内容去重。
- 不在首次改造中改变当前 50 MiB 限制；流式链路稳定后再通过策略配置调整。
...
对象键只使用服务端 UUID，不使用原始文件名。建议布局：

```text
<root>/objects/v1/ab/cd/<attachment-uuid>
...
```text
<configured-prefix>/objects/v1/<attachment-uuid>
```
...
当前只有数据库路径环境变量。建议新增版本化 TOML 配置，同时允许敏感字段引用环境变量。
示例仅表示目标结构：
...
- 实现 OSS 适配器、multipart 清理和故障注入测试。
...
- 默认不做内容寻址去重：对象键使用 UUID，SHA-256 用于完整性。若未来启用去重，需要单独设计
  引用计数、跨房间信息泄漏、加密和保留语义。
