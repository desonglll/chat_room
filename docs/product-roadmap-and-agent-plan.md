...
### BUG-AI-003 视觉上下文覆盖与质量

- **基线/Owner**：`b6a2805`；当前 AI/Knowledge 垂直任务 owner。
- **用户结果**：房间总结不会静默漏掉图片；图片内容可参与后续检索，回答能将视觉事实绑定回原消息，并明确展示跳过或失败数量。
- **实现**：新增 Room-scoped 附件视觉投影与 `20260901030005` 双数据库迁移；分批处理受预算约束的完整图片集合；视觉投影按原消息授权读取、写回 Message Vector，并以结构化来源映射进入最终上下文。
- **接口**：不新增 HTTP/WebSocket 接口；沿用 AI Run trace，新增隐私安全的候选、缓存命中、成功及失败计数。
- **允许路径**：AI/Knowledge 域实现与测试、配置和配置文档、匹配的 SQLite/PostgreSQL 迁移；不修改共享模型、路由、Web API 或锁文件。
- **验收**：覆盖超过单批上限的总结、缓存复用与失效、双数据库授权、向量文本更新、部分失败可见性，以及最终 prompt 的消息级视觉绑定。
