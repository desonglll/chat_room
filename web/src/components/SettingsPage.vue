...
<template>
  <main class="min-h-0 min-w-0 flex-1 overflow-y-auto bg-surface-0">
    <header class="sticky top-0 z-10 flex h-[72px] items-center gap-3 border-b border-surface-200 bg-surface-0/95 px-4 backdrop-blur sm:px-7">
      ...
      <div><h2 class="text-base font-semibold">设置</h2><p class="mt-0.5 text-xs text-muted-color">账户与应用偏好</p></div>
    </header>

    <div class="mx-auto w-full max-w-2xl px-5 py-8 sm:px-8">
      <section class="border-b border-surface-200 pb-7">
        <div class="flex items-center justify-between gap-4">
          <div class="flex items-center gap-3"><Settings :size="19" class="text-primary" /><div><strong class="block text-sm">聊天与通知偏好</strong><small class="text-muted-color">快捷键、浏览器通知与消息详情</small></div></div>
      ...
      <form autocomplete="on" class="space-y-5 border-b border-surface-200 py-7" @submit.prevent="savePassword">
        ...
        <div class="flex justify-end"><Button type="submit" :loading="passwordSaving">保存新密码</Button></div>
      </form>
