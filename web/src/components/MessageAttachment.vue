...
<template>
  <div
    class="relative w-[min(78vw,440px)] overflow-hidden rounded-lg border border-surface-200 bg-surface-0 sm:w-[min(72vw,440px)]"
    :class="{ 'min-h-28': attachment.is_sensitive && !revealed }"
  >
    <div :class="{ 'blur-xl': attachment.is_sensitive && !revealed }">
      <button
      ...
      />
      <a
        class="flex min-h-14 items-center gap-2.5 px-3 py-2 text-surface-700 no-underline hover:bg-surface-50"
        :class="{ 'pointer-events-none': attachment.is_sensitive && !revealed }"
