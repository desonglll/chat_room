...
<template>
  <header class="flex h-[72px] shrink-0 items-center justify-between gap-3 border-b border-surface-200 bg-surface-0 px-3 sm:px-5">
    <div class="flex min-w-0 items-center gap-2 sm:gap-3">
      ...
      <div class="min-w-0">
        <div class="flex min-w-0 items-center gap-1.5">
          <h2 class="truncate text-[15px] font-semibold text-surface-900" :title="room.description || undefined">{{ room.name }}</h2>
          <Button text rounded severity="secondary" size="small" :aria-label="roomIdCopied ? '聊天室 ID 已复制' : '复制聊天室 ID'" :title="roomIdCopied ? '已复制' : `复制 ID：${room.id}`" @click="copyRoomId">
    ...
    <div class="flex shrink-0 items-center gap-1">
      <Popover ref="memberPopover">
        <div class="w-60">
          ...
          <ul class="max-h-72 space-y-1 overflow-y-auto p-0">
            <li v-for="member in members" :key="member.user_id" class="group flex min-h-10 items-center gap-2.5 rounded-md px-1.5 py-1 text-sm hover:bg-surface-50">
              <button type="button" class="min-w-0 cursor-pointer rounded-full" aria-label="查看用户资料" @click="emit('viewProfile', member.user_id)">
                <Avatar
              ...
              <span class="min-w-0 flex-1 truncate">{{ member.user_id === currentUserId ? `${member.username}（你）` : member.username }}</span>
              <button
      ...
      <Button v-if="authenticated" text rounded severity="secondary" aria-label="聊天文件" title="聊天文件" @click="emit('files')">
