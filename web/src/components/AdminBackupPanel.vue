...
import { ArchiveRestore, DatabaseBackup, Download, FileArchive, Upload } from 'lucide-vue-next'
...
const supported = computed(() => props.databaseBackend === 'postgres')
const fileModeAvailable = computed(() => supported.value && props.attachmentBackend === 'local')

async function downloadBackup(): Promise<void> {
  if (!supported.value || (scope.value === 'complete' && !fileModeAvailable.value)) return
...
<template>
  <section aria-labelledby="backup-heading" class="mt-10 border-t border-surface-200 pt-7">
    ...
    <Message v-if="!supported" severity="warn" :closable="false" class="mb-5">
      当前 SQLite 模式不支持在线备份与恢复。
    </Message>
    <Message v-else-if="attachmentBackend === 'oss'" severity="info" :closable="false" class="mb-5">
    ...
    <div class="grid gap-7 lg:grid-cols-2 lg:gap-10">
      <div class="min-w-0">
        ...
        <Button
          class="mt-3"
          :disabled="!supported || (scope === 'complete' && !fileModeAvailable)"
          :loading="exporting"
      ...
      <div class="min-w-0 border-t border-surface-200 pt-7 lg:border-l lg:border-t-0 lg:pl-10 lg:pt-0">
        ...
        />
        <Button class="mt-3" severity="warning" outlined :disabled="!supported" @click="fileInput?.click()">
          <Upload :size="17" aria-hidden="true" />选择备份归档
        </Button>
      </div>
    </div>
  </section>
